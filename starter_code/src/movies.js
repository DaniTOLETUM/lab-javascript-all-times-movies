...
function orderByDuration(m) {
	function order(a, b) {
		return a.duration === b.duration ? (a.title > b.title ? 1 : -1) : a.duration > b.duration ? 1 : -1;
...
function howManyMovies(a) {
	const $PIELBERG = a.filter((x) => x.director === 'Steven Spielberg' && x.genre.includes('Drama'));
...
function bestYearAvg(movies) {
	...
	for (let i = 0; i < globalArr.length; i++) {
		let currentYearAverage = globalArr[i].reduce((acc, current) => (acc += +current.rate), 0);
		if (+(currentYearAverage / globalArr[i].length).toFixed(2) > bestYearAverage) {
